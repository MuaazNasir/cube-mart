  ...
  useEffect(() => {
    const fetchData = async () => {
      const categories = await getData(
        ` *[ _type == 'categories' ]{name,image} `
      );
      const products = await getData(
        ` *[ _type == 'product' ]{...,'category':category[]->name}`
      );
  ...
  return (
    <>
      <div className="space-y-5">
        <div className="text-5xl text-center text-black font-sans font-extrabold">
          Categories
        </div>
        <div className="flex flex-row flex-wrap bg-gray-50 !px-4 !py-5">
          {categoryProducts &&
            categoryProducts.map((category, i) => {
              return (
                <Fragment key={i}>
                  <div className="w-full text-3xl font-sans my-3 font-extrabold text-center capitalize">
