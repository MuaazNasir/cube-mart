    ...
    <>
      <div className="w-full max-w-sm bg-gray-200 border border-gray-200 rounded-lg m-5 transition-all hover:shadow-gray-400 shadow-md">
        <Toaster
          ...
          toastOptions={{
            className:
              "bg-green-300 text-lg text-white font-bold tracking-wide",
          }}
        />

        <Link href={`/products/${slug.current}`} passHref>
          <Image
            className="p-8 rounded-t-lg mx-auto hover:scale-105 transition-all"
            src={urlFor(image).size(250, 250).url()}
            height={250}
            width={250}
            alt="product image"
          />
        ...
        <div className="px-5 pb-5">
          ...
          <div className="flex items-center mt-2.5 mb-5 cursor-default">
            {ratingArr.map((elem, i) => {
              return (
                <Fragment key={i}>
                  <svg
                    className={`w-4 h-4 ${
                      rating - 1 >= i ? "text-yellow-300" : "text-gray-400"
                    } mr-1`}
                    aria-hidden="true"
            ...
            <span className="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded ml-3 cursor-default">
              {rating}.0
            </span>
          </div>
          <div className="flex flex-row items-center justify-center gap-5 flex-wrap ">
            <span className="text-3xl font-bold text-gray-900 cursor-default">
              ${price}
            </span>
            <button
