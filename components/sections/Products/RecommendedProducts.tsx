...
const RecommendedProducts = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const products = await getData(` *[_type == "product" && recommended == true][0..${noOfProducts - 1}] `);
                ...
                <div className="flex flex-row flex-wrap justify-center items-center">
                    {
                        products.map((e: any) => {
                            ...
                            return (
                                <>
                                    <ProductCard name={name} price={price} image={image[0]} rating={4} slug={slug} _id={_id}/>
                                </>
                            )
                        }
                        )
